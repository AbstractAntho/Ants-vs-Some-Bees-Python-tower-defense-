...
class Place:
    ...
    def __init__(self, name: str, exit: Place | None = None):
        ...
        name -- A string; the name of this Pla#ce.
...
class Ant(Insect):
    ...
    def double(self):
        ...
        if hasattr(self, 'damage') and not getattr(self, 'damage_doubled', False):
            self.damage *= 2
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount: float):
        ...
        reflect = amount
        dying = (self.health - amount) <= 0
        if dying:
            reflect += self.damage

        if self.place is not None:
            for bee in list(self.place.bees):
                bee.reduce_health(reflect)
...
class HungryAnt(Ant):
    ...
    food_cost = 4
    implemented = True
    chew_cooldown = 3   # number of turns to chew

    def __init__(self, health=1):
        super().__init__(health)
        self.cooldown = 0    # turns left to chew

    def action(self, gamestate):
        # If currently chewing, decrement
        if self.cooldown > 0:
            self.cooldown -= 1
            return

        # If not chewing, try to eat a Bee
        if self.place and self.place.bees:
            bee = random_bee(self.place.bees)
            if bee:
                bee.reduce_health(bee.health)   # eat the bee
